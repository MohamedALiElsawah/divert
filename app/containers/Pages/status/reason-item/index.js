...
import { storeReasonsItems, setLoading } from '../actions';
import { getReasonItemsApi, deleteReasonItemApi } from '../apis';
...
function ClientList(props) {
  ...
  const getReasonsItems = () => {
    getReasonItemsApi().then(res => {
      storeReasonsItems(res.data);
    })
      .catch(err => { });
  };
  useEffect(() => {
  ...
  const handleDeleteBlock = (id) => {
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          setLoading(true);
          deleteReasonItemApi(id).then(res => {
            setLoading(false);
            getReasonsItems();
            swal({
              title: 'Success',
              text: 'Reason Item deleted!',
              icon: 'success',
            });
          })
            .catch(err => {
          ...
          <TableBody>
            {reasonsItems.map(item => ([
              <TableRow key={item.id} className={classNames(classes.pointer)}>
                ...
                <TableCell align="left">{reasons.find(reason => reason.id == item.reason_id) && reasons.find(reason => reason.id == item.reason_id).name}</TableCell>
                <TableCell align="left">
                  ...
                  <Fab
                    ...
                    onClick={() => {
                      ...
                      history.push(`/app/reason-item/${item.id}/edit`);
                    }}
                  >
                    <EditIcon />
                  </Fab>
...
ClientList.propTypes = {
  ...
  storeReasonsItems: PropTypes.func.isRequired,
...
const mapStateToProps = state => ({
  ...
  reasonsItems: state.get(reducer).reasonsItems,
  reasons: state.get(lookupsReducer).reasons,
