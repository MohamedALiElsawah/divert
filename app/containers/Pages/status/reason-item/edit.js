...
import { editReasonItemApis, getReasonItemDetailsApis } from '../apis'
...
function EditForm(props) {
  ...
  useEffect(() => {
    ...
    getReasonItemDetailsApis(id).then(res => {
      // (res.data.data)
  ...
  const onSubmit = (valueForm) => {
    ...
    let obj = {
      ...
      "status": 1,
      "reason_id":formObj.reason_id

    }

    setLoading(true)
    editReasonItemApis(id, obj).then(res => {
      setLoading(false)
      handleSuccessMessage('Reason Updated successfully!')
    })
      .catch(error => {
        setLoading(false)
      ...
      <section>
        ...
        <Grid container spacing={3}>
            <Grid item xs={12} sm={6}>
              <TextValidator
                placeholder='Enter Reason English name'
                fullWidth
                variant="outlined"
                InputLabelProps={{
                  shrink: true,
            ...
            <Grid item xs={12} sm={6}>
              <TextValidator
                ...
                fullWidth
                label="Arabic Name"
                placeholder='Enter Reason Arabic name'
